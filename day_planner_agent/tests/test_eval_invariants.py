"""Unit coverage of evals/invariants.py's tier-1 predicates — pure
functions over fixture data, no model calls, so these run in the normal
fast pytest suite even though the eval scenarios themselves (evals/
runner.py) need a real model and are invoked separately.
"""

from day_planner_agent.evals import invariants as inv


def _world(*, zones=None, sleep_schedule=None, habits=None) -> inv.World:
    return inv.World(zones=zones or [], sleep_schedule=sleep_schedule, habits=habits or [])
